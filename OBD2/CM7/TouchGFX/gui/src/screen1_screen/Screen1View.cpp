...
Screen1View::Screen1View() :
		boxWithBorder1ClickedCallback(this,
				&Screen1View::boxWithBorder1ClickHandler), Termination_Resistor_Callback(
				this, &Screen1View::Resistor_icon_ClickHandler) {

}

void Screen1View::setupScreen() {
	Screen1ViewBase::setupScreen();
	boxWithBorder1.setClickAction(boxWithBorder1ClickedCallback);
	Resistor_icon.setClickAction(Termination_Resistor_Callback);
	uint8_t sample_size = 45;
...
void Screen1View::Resistor_icon_ClickHandler(const ScalableImage &b,
		const ClickEvent &e) {
...
void Screen1View::boxWithBorder1ClickHandler(const BoxWithBorder &b,
		const ClickEvent &e) {
	if (&b == &boxWithBorder1) {
		plot_graphs();
	}
...
void Screen1View::update_values_exe() {
	...
	if (xQueueReceive(Sensor_Queue, &SensorValues, pdMS_TO_TICKS(10)) == pdPASS) {
		Unicode::snprintfFloat(Voltage_textBuffer, VOLTAGE_TEXT_SIZE, "%.2f V",
				SensorValues.voltage);
		Unicode::snprintfFloat(Current_textBuffer, CURRENT_TEXT_SIZE, "%.2f A",
